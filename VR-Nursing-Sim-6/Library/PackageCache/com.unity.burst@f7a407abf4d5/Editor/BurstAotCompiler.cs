...
namespace Unity.Burst.Editor
{
    ...
    internal class StaticPreProcessor : IPreprocessBuildWithReport
    {
        ...
        public void OnPreprocessBuild(BuildReport report)
        {
            ...
            ||(report.summary.platform == BuildTarget.Switch2)
#endif            
            )
            {
        ...
        internal static IEnumerable<string> OnPostBuildPlayerScriptDLLsImpl(BurstAOTSettings settings, Assembly[] playerAssemblies)
        {
            ...
            foreach (var combination in settings.combinations)
            {
                ...
                if (settings.targetPlatform == TargetPlatform.iOS || settings.targetPlatform == TargetPlatform.tvOS
                 || settings.targetPlatform == TargetPlatform.Switch || settings.targetPlatform == TargetPlatform.visionOS
                 || settings.targetPlatform == TargetPlatform.visionSimulator || settings.targetPlatform == TargetPlatform.WASM
#if UNITY_6000_3_OR_NEWER
                    || settings.targetPlatform == TargetPlatform.Switch2
#endif                
                )
                {
...
";
        ...
        internal static List<BurstOutputCombination> CollectCombinations(TargetPlatform targetPlatform, TargetCpus targetCpus, BuildSummary summary, ScriptingImplementation scriptingImplementation)
        {
...
#if UNITY_6000_3_OR_NEWER            
                ||(targetPlatform == TargetPlatform.Switch2)
#endif
            )
            {
            ...
            {
                // We currently don't support WebGL/Wasm builds
            }
            else
            {
                // Safeguard
