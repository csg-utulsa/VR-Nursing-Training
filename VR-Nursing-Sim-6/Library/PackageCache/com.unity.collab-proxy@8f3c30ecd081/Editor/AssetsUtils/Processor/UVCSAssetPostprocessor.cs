...
{
    class UVCSAssetPostprocessor : AssetPostprocessor
    {
            ...
            UVCSAssetsProcessor uvcsAssetsProcessor)
        {
            ...
            mWkPath = wkPath;
            mUVCSAssetsProcessor = uvcsAssetsProcessor;
            ...
            string[] movedFromAssetPaths)
        {
            ...
            // 3. Open UVCS window, the asset should appear as added instead of deleted locally.
            UVCSPlugin.EnableMonoFsWatcherIfNeeded();

            List<PathToMove> pathsToMove = GetPathsToMoveOnWorkspace(
                mWkPath, movedAssets, movedFromAssetPaths);

            mUVCSAssetsProcessor.MoveOnSourceControl(pathsToMove);
            ...
            mUVCSAssetsProcessor.CheckoutOnSourceControl(pathsToCheckout);

            mUVCSAssetsProcessor.DeleteFromSourceControl(
                GetPathsContainedOnWorkspace(mWkPath, deletedAssets));

            mUVCSAssetsProcessor.AddToSourceControl(pathsToAdd);

            // Unity Editor notifies the modified assets through
            // UVCSAssetModificationProcessor.OnWillSaveAssets before getting here.
            // Known Issue: renamed prefabs not triggering OnWillSaveAssets.
            mUVCSAssetsProcessor.CheckoutOnSourceControl(
                GetPathsContainedOnWorkspace(
                    mWkPath, UVCSAssetModificationProcessor.ExtractModifiedAssetsToProcess()));
        }
        ...
        static UVCSAssetsProcessor mUVCSAssetsProcessor;
        static string mWkPath;

        static readonly ILog mLog = PlasticApp.GetLogger("UVCSAssetPostprocessor");
    }
}
