...
{
    internal class UVCSAssetsProcessor :
        WorkspaceOperationsMonitor.IDisableAssetsProcessor
    {
        ...
        internal void AddToSourceControl(List<string> paths)
        {
            ...
            {
                mLog.Warn(
                    "UVCSAssetsProcessor skipping AddToSourceControl operation " +
        ...
        internal void DeleteFromSourceControl(List<string> paths)
        {
            ...
            {
                mLog.Warn(
                    "UVCSAssetsProcessor skipping DeleteFromSourceControl operation " +
                    "because an exception happened in the last 60 seconds");
                return;
            }
        ...
        internal void MoveOnSourceControl(List<UVCSAssetPostprocessor.PathToMove> paths)
        {
            if (paths.Count == 0)
                return;
            ...
            {
                mLog.Warn(
                    "UVCSAssetsProcessor skipping MoveOnSourceControl operation " +
                    "because an exception happened in the last 60 seconds");
                return;
            }

            foreach (UVCSAssetPostprocessor.PathToMove path in paths)
        ...
        internal void CheckoutOnSourceControl(List<string> paths)
        {
            ...
            {
                mLog.Warn(
                    "UVCSAssetsProcessor skipping CheckoutOnSourceControl operation " + 
                    "because an exception happened in the last 60 seconds");
                return;
            }
        ...
        static readonly ILog mLog = PlasticApp.GetLogger("UVCSAssetsProcessor");
    }
}
